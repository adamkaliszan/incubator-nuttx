/****************************************************************************
 * arch/arm/src/stm32wl5/stm32wl5_dma.h
 *
...
#ifndef __ARCH_ARM_SRC_STM32WL5_STM32WL5_DMAWL5_H
#define __ARCH_ARM_SRC_STM32WL5_STM32WL5_DMAWL5_H
...
#include "hardware/stm32wl5_dma.h"

#ifdef CONFIG_STM32WL5_HAVE_DMAMUX
#  include "hardware/stm32wl5_dmamux.h"
#endif
...
DMA_HANDLE stm32wl5_dmachannel(unsigned int chan);

/****************************************************************************
 * Name: stm32wl5_dmafree
 *
 * Description:
 *   Release a DMA channel.  If another thread is waiting for this DMA
 *   channel in a call to stm32wl5_dmachannel, then this function will re-
 *   assign the DMA channel to that thread and wake it up.
 *
 *   NOTE:  The 'handle' used in this argument must NEVER be used again
 *   until stm32wl5_dmachannel() is called again to re-gain access to the
 *   channel.
 *
 * Returned Value:
 *   None
...
void stm32wl5_dmafree(DMA_HANDLE handle);

/****************************************************************************
 * Name: stm32wl5_dmasetup
 *
 * Description:
...
void stm32wl5_dmasetup(DMA_HANDLE handle, uint32_t paddr, uint32_t maddr,
                    size_t ntransfers, uint32_t ccr);

/****************************************************************************
 * Name: stm32wl5_dmastart
 *
 * Description:
...
void stm32wl5_dmastart(DMA_HANDLE handle, dma_callback_t callback, void *arg,
                    bool half);

/****************************************************************************
 * Name: stm32wl5_dmastop
 *
 * Description:
...
void stm32wl5_dmastop(DMA_HANDLE handle);

/****************************************************************************
 * Name: stm32wl5_dmaresidual
 *
 * Description:
...
size_t stm32wl5_dmaresidual(DMA_HANDLE handle);

/****************************************************************************
 * Name: stm32wl5_dmacapable
 *
 * Description:
...
bool stm32wl5_dmacapable(uintptr_t maddr, uint32_t count, uint32_t ccr);
#else
#  define stm32wl5_dmacapable(maddr, count, ccr) (true)
#endif

/****************************************************************************
 * Name: stm32wl5_dmasample
 *
 * Description:
...
#ifdef CONFIG_DEBUG_DMA_INFO
void stm32wl5_dmasample(DMA_HANDLE handle, struct stm32wl5_dmaregs_s *regs);
#else
#  define stm32wl5_dmasample(handle,regs)
#endif

/****************************************************************************
 * Name: stm32wl5_dmadump
 *
 * Description:
...
#if defined(HAVE_IP_DMA_V1)
uint32_t stm32wl5_dma_intget(unsigned int chndx);
#elif defined(HAVE_IP_DMA_V2)
uint8_t  stm32wl5_dma_intget(unsigned int controller, uint8_t stream);
#endif
...
#endif /* __ASSEMBLY__ */
#endif /* __ARCH_ARM_SRC_STM32WL5_STM32WL5_DMA_H */
